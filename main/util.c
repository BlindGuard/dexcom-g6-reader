...
void print_bytes_debug(uint8_t *bytes, int len) {
    int i;
...
void print_adv_fields(struct ble_hs_adv_fields *adv_fields) {
    ...
    MODLOG_DFLT(DEBUG, "[=========== Received advertisement ============]");
    if(adv_fields->name != NULL) {
        MODLOG_DFLT(DEBUG, "Device name                (%scomplete) : \n",
    ...
    if(adv_fields->public_tgt_addr != NULL) {
        MODLOG_DFLT(DEBUG, "Public target address                   : \n");
    ...
    if(adv_fields->uuids16 != NULL) {
        MODLOG_DFLT(DEBUG, "16-bit service class UUIDs (%scomplete) : \n",
    ...
    if(adv_fields->uuids32 != NULL) {
        MODLOG_DFLT(DEBUG, "32-bit service class UUIDs (%scomplete) : \n",
    ...
    if(adv_fields->uuids128 != NULL) {
        MODLOG_DFLT(DEBUG, "128-bit service class UUIDs (%scomplete) : \n",
    ...
    if(adv_fields->svc_data_uuid16 != NULL) {
        MODLOG_DFLT(DEBUG, "16-bit service data                     : \n\t");
    ...
    if(adv_fields->svc_data_uuid32 != NULL) {
        MODLOG_DFLT(DEBUG, "32-bit service data                     : \n\t");
    ...
    if(adv_fields->svc_data_uuid128 != NULL) {
        MODLOG_DFLT(DEBUG, "128-bit service data                    : \n\t");
